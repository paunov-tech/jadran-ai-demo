...
const RAB_SEED = {
  pois: [
    { name: "Rab centar parking", category: "parking", address: "Palit bb, Rab", lat: "44.7575", lng: "14.7646", hours: "00-24", phone: "", price: "1.50\u20ac/h", desc_hr: "Glavni parking blizu starog grada", desc_en: "Main parking near old town", desc_de: "Hauptparkplatz nahe Altstadt" },
    { name: "Pla\u017ea Pudarica", category: "beach", address: "Pudarica, Rab", lat: "44.7400", lng: "14.7890", hours: "", phone: "", price: "", desc_hr: "Pje\u0161\u010dana pla\u017ea, plitko more, idealna za obitelji", desc_en: "Sandy beach, shallow water, perfect for families", desc_de: "Sandstrand, flaches Wasser, ideal f\u00fcr Familien" },
    { name: "Konoba Rab", category: "restaurant", address: "Kneza Branimira 3, Rab", lat: "44.7570", lng: "14.7610", hours: "11:00-23:00", phone: "+385 51 725 666", price: "\u20ac\u20ac", desc_hr: "Tradicionalna dalmatinska konoba s pogledom na more", desc_en: "Traditional Dalmatian tavern with sea view", desc_de: "Traditionelle dalmatinische Konoba mit Meerblick" },
    { name: "Ljekarna Rab", category: "pharmacy", address: "Mali Palit 79, Rab", lat: "44.7565", lng: "14.7625", hours: "07:30-20:00", phone: "+385 51 724 177", price: "", desc_hr: "Jedina ljekarna na otoku", desc_en: "Only pharmacy on the island", desc_de: "Einzige Apotheke auf der Insel" },
    { name: "Studenac Market", category: "shop", address: "Palit bb, Rab", lat: "44.7560", lng: "14.7635", hours: "06:30-21:00", phone: "", price: "", desc_hr: "Najve\u0107i supermarket na otoku", desc_en: "Largest supermarket on island", desc_de: "Gr\u00f6\u00dfter Supermarkt der Insel" },
    { name: "ACI Marina Rab", category: "marina", address: "Obala Petra Kre\u0161imira IV", lat: "44.7585", lng: "14.7640", hours: "08:00-20:00", phone: "+385 51 724 023", price: "Vez od 50\u20ac/no\u0107", desc_hr: "142 veza, voda, struja, WiFi", desc_en: "142 berths, water, electricity, WiFi", desc_de: "142 Liegepl\u00e4tze, Wasser, Strom, WiFi" },
    { name: "Katedrala sv. Marije", category: "culture", address: "Gornja ulica, Rab", lat: "44.7578", lng: "14.7605", hours: "09:00-18:00", phone: "", price: "3\u20ac", desc_hr: "Romani\u010dka katedrala iz 12. stolje\u0107a", desc_en: "12th century Romanesque cathedral", desc_de: "Romanische Kathedrale, 12. Jh." },
    { name: "Pekara Rab", category: "bakery", address: "Municipium Arba 8", lat: "44.7572", lng: "14.7612", hours: "05:30-20:00", phone: "", price: "", desc_hr: "Rapska torta \u2014 lokalni specijalitet!", desc_en: "Rab cake \u2014 local specialty!", desc_de: "Rab-Torte \u2014 lokale Spezialit\u00e4t!" },
  ],
  events: [
    { name: "Rapska fjera", date: "2026-07-25", time: "18:00", location: "Stari grad Rab", category: "kultura", capacity: "2000", price: "Besplatno", desc: "Srednjovjekovna tradicija \u2014 vitezovi, zanatlije, glazba" },
    { name: "Ribarska no\u0107", date: "2026-08-10", time: "20:00", location: "Riva, Rab", category: "gastro", capacity: "500", price: "Besplatno", desc: "Lokalna riba na \u017earu, vino, glazba u\u017eivo" },
    { name: "Rab Summer Festival", date: "2026-07-15", time: "21:00", location: "Gradski park", category: "muzika", capacity: "800", price: "15\u20ac", desc: "Koncerti i ljetna atmosfera" },
  ],
  analytics: { sessions: "47", monthly_sessions: "312", ai_queries: "89", nav_clicks: "156", top_lang: "DE" },
};
...
export default function TZDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const myPois = p.filter(x => x.tz_id === tzId || !x.tz_id);
      const myEvents = e.filter(x => x.tz_id === tzId || !x.tz_id);

      // Auto-seed Rab demo data if empty
      if (tzId === "tz_rab" && myPois.length === 0 && RAB_SEED.pois.length > 0) {
        const seeded = [];
        ...
        for (const evt of RAB_SEED.events) {
          const docId = `evt_rab_${Date.now()}_${Math.random().toString(36).slice(2,6)}`;
          await firestoreWrite("jadran_events", docId, { ...evt, tz_id: tzId, created: new Date().toISOString() });
        }
        setEvents(RAB_SEED.events.map((ev, i) => ({ id: `evt_rab_${i}`, ...ev, tz_id: tzId })));

        await firestoreWrite("jadran_analytics", `analytics_rab_${Date.now()}`, { ...RAB_SEED.analytics, tz_id: tzId, date: new Date().toISOString().split("T")[0] });
        setStats(RAB_SEED.analytics);
      } else {
        setPois(myPois);
        setEvents(myEvents);
        if (a.length > 0) {
  ...
  }, [tzId, firestoreRead, firestoreWrite]);
  ...
  const renderContent = () => {
    ...
    switch (screen) {
      ...
      case "sense":
        return (
          <>
            <div style={{ ...hf, fontSize: 28, fontWeight: 400, marginBottom: 8 }}>YOLO Sense</div>
            <div style={{ ...dm, fontSize: 13, color: C.mut, marginBottom: 20 }}>Senzorska platforma — parking, marina, plaža u realnom vremenu</div>

            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14, marginBottom: 24 }}>
              <Card style={{ textAlign: "center", padding: "20px 16px" }}>
                <div style={{ fontSize: 36 }}>🅿️</div>
                <div style={{ fontSize: 32, fontWeight: 300, color: C.green, marginTop: 4 }}>24/30</div>
                <div style={{ ...dm, fontSize: 11, color: C.mut, marginTop: 2 }}>Slobodnih mjesta</div>
                <div style={{ ...dm, fontSize: 10, color: C.accent, marginTop: 6 }}>Parking {tzName}</div>
              </Card>
              <Card style={{ textAlign: "center", padding: "20px 16px" }}>
                <div style={{ fontSize: 36 }}>⛵</div>
                <div style={{ fontSize: 32, fontWeight: 300, color: C.accent, marginTop: 4 }}>8</div>
                <div style={{ ...dm, fontSize: 11, color: C.mut, marginTop: 2 }}>Slobodnih vezova</div>
                <div style={{ ...dm, fontSize: 10, color: C.accent, marginTop: 6 }}>Marina {tzName}</div>
              </Card>
              <Card style={{ textAlign: "center", padding: "20px 16px" }}>
                <div style={{ fontSize: 36 }}>🏖️</div>
                <div style={{ fontSize: 32, fontWeight: 300, color: C.gold, marginTop: 4 }}>35%</div>
                <div style={{ ...dm, fontSize: 11, color: C.mut, marginTop: 2 }}>Popunjenost plaže</div>
                <div style={{ ...dm, fontSize: 10, color: C.green, marginTop: 6 }}>mirno</div>
              </Card>
              <Card style={{ textAlign: "center", padding: "20px 16px" }}>
                <div style={{ fontSize: 36 }}>👥</div>
                <div style={{ fontSize: 32, fontWeight: 300, color: C.text, marginTop: 4 }}>312</div>
                <div style={{ ...dm, fontSize: 11, color: C.mut, marginTop: 2 }}>Aktivnih turista</div>
                <div style={{ ...dm, fontSize: 10, color: C.mut, marginTop: 6 }}>danas</div>
              </Card>
            </div>

            <SectionLabel>Sense lokacije</SectionLabel>
            <Card style={{ marginBottom: 14 }}>
              {[
                { name: `Parking ${tzName} centar`, type: "parking", status: "slobodno", value: "24/30", clr: C.green },
                { name: `Marina ${tzName}`, type: "marina", status: "umjereno", value: "8 vezova", clr: C.gold },
                { name: `Glavna plaža`, type: "beach", status: "mirno", value: "35%", clr: C.green },
                { name: `Riva`, type: "pedestrian", status: "malo gužve", value: "~120", clr: C.accent },
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: i < 3 ? `1px solid ${C.bord}` : "none" }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 500 }}>{s.name}</div>
                    <div style={{ ...dm, fontSize: 11, color: C.mut }}>{s.type}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ ...dm, fontSize: 14, fontWeight: 600, color: s.clr }}>{s.value}</div>
                    <div style={{ ...dm, fontSize: 10, color: s.clr }}>{s.status}</div>
                  </div>
                </div>
              ))}
            </Card>

            <SectionLabel>O YOLO Sense</SectionLabel>
            <Card>
              <div style={{ ...dm, fontSize: 13, color: C.mut, lineHeight: 1.8 }}>
                YOLO Sense je senzorska platforma koja prati popunjenost parkinga, marina i plaža u realnom vremenu.
                Podaci se ažuriraju svakih 10 minuta i prikazuju turistima u kiosk modu.
                <br /><br />
                Platforma NE koristi video nadzor niti kamere — podaci dolaze iz senzora i algoritama za procjenu gustoće.
              </div>
            </Card>
          </>
        );
