...
// Primary: LIVE YOLO crowd data from Firestore (same system as api/chat.js)
...
// ─── YOLO Firestore fetch (mirrored from api/chat.js — Vercel functions can't share modules) ───
let _yoloCache = null;
let _yoloCacheTime = 0;
const YOLO_CACHE_MS = 5 * 60 * 1000;
...
async function fetchYoloCrowd() {
  ...
  try {
    ...
    console.error("camera.js YOLO fetch error:", e.message);
    return null;
  }
}
...
export default async function handler(req, res) {
  ...
  let data = null;
  try {
    const yolo = await fetchYoloCrowd();
    if (yolo) data = yoloToBeachStatus(yolo);
  } catch (e) {
    console.warn("camera.js: YOLO unavailable, falling back to mock:", e.message);
  }
