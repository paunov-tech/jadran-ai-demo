...
export default async function handler(req, res) {
  ...
  if (yolo) {
    if (yolo.activeCams === 0) {
      context += `\n\nYOLO CAMERAS: Night mode — 0 active cameras of 166. Cameras inactive at night, do NOT interpret as "no traffic".`;
    } else {
      context += `\n\nYOLO CAMERAS (${yolo.activeCams} active of 166):`;
...
async function fetchNearbyYolo(lat, lng) {
  if (YOLO_CACHE.data && Date.now() - YOLO_CACHE.ts < YOLO_CACHE_TTL) return YOLO_CACHE.data;
